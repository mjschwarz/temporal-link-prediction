...
def get_args():
    parser = argparse.ArgumentParser('Temporal Link Prediction')
...
class TemporalEncoding(nn.Module):
    ...
    def forward(self, x):
        inp = x.repeat(self.bits, 1).transpose(0, 1)
        div = torch.cat([torch.ones((x.shape[0], 1)) * 10 ** (self.bits - 1 - i) for i in range(self.bits)], 1)
        return (((inp / div).int() % 10) * 0.1).float()

class TemporalHeteroConv(nn.Module):
    def __init__(self, etypes, n_layers, in_feats, hid_feats, activation, bits, dropout=0.2):
        super(TemporalHeteroConv, self).__init__()
        self.etypes = etypes
        ...
        self.temporal_encoder = TemporalEncoding(bits)
        ...
        self.hconv_layers.append(self.build_hconv(hid_feats, hid_feats)) # No activation in output layer        

        self.fc1 = nn.Linear(hid_feats * 2 + self.bits, hid_feats)
...
def train(args, g):
    ...
    bits=10
    model = TemporalHeteroConv(g.etypes, args.n_layers, dim_nfeat, args.emb_dim, F.relu, bits)
    ...
    for epoch in range(args.epochs):
        ...
        for etype in g.etypes:
            ...
            time_emb = model.time_encode(ts)  
            time_emb_shuffle = model.time_encode(ts_shuffle) 
    ...
    # Plot AUC Score if available
    if auc_scores:
        plt.subplot(1, 2, 2)
        plt.plot(range(0, args.epochs, 10), auc_scores, label='AUC Score', marker='o')
        plt.xlabel('Epochs')
        plt.ylabel('AUC Score')
        plt.title('AUC Score at Testing Intervals')
        plt.legend()

    plt.tight_layout()
    plt.show()
    return g, model  # Return the graph and model
...
def test(args, g, model):
    ...
    start_time_emb = model.time_encode(start_at)
    end_time_emb = model.time_encode(end_at)
